...
namespace RemSolution.Application.Features.Statistics.Queries
{
    /// <summary>
    /// The statistics figures, folded once. The screen's query and the export both
    /// come through here — an export that re-derived the numbers would be a second
    /// place for the attribution rules to drift, and a spreadsheet disagreeing with
    /// the screen it was downloaded from is the one defect nobody forgives.
    /// </summary>
    internal static class StatisticsReport
    {
        ...
        public static async Task<StatisticsDto> BuildAsync(
        ...
        {
            var agencyId = tenant.AgencyId ?? throw new UnauthorizedAccessException();
            var currency = (await settings.GetAsync(agencyId, cancellationToken)).CurrencyCode;

            var now = dateTime.GetUtcNow().UtcDateTime;

            var (from, to, truncated) = ResolveWindow(requestedFrom, requestedTo, granularity, now);
            ...
            var hires = await context.Rentings
